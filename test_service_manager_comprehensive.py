...
Tests every aspect of the service manager functionality
"""
...
class ServiceManagerTestSuite:
    ...
    def wait_for_service_manager(self, max_wait=60):
        """Wait for service manager to be ready"""
        print("⏳ Waiting for service manager to be ready...")
        ...
        while time.time() - start_time < max_wait:
            try:
                ...
                if result == 0:
                    # Port is open, test if service responds
                    response = requests.get(f"{self.base_url}/status", timeout=2)
                    if response.status_code == 200:
                        print(f"✅ Service manager ready after {time.time() - start_time:.1f} seconds")
                        return True
            except:
                pass
        ...
        print(f"❌ Service manager not ready after {max_wait} seconds")
        return False
    
    def start_service_manager(self):
        """Start the service manager"""
        print("\n🚀 Starting MediVote Service Manager...")
        try:
            ...
            # Wait for service manager to be ready
            if self.wait_for_service_manager():
                self.log_test("STARTUP", "Service Manager Process", True, "Process started successfully")
                return True
            else:
                stdout, stderr = self.service_manager_process.communicate()
                self.log_test("STARTUP", "Service Manager Process", False, f"Process not ready: {stderr}")
                return False
        except Exception as e:
            self.log_test("STARTUP", "Service Manager Process", False, error=e)
            return False
    ...
    def stop_service_manager(self):
        ...
        if self.service_manager_process:
            try:
                ...
                self.service_manager_process.wait(timeout=10)
                self.log_test("SHUTDOWN", "Service Manager Shutdown", True)
            except subprocess.TimeoutExpired:
                self.service_manager_process.kill()
                self.log_test("SHUTDOWN", "Service Manager Force Kill", True)
            except Exception as e:
                self.log_test("SHUTDOWN", "Service Manager Shutdown", False, error=e)
    ...
    def run_all_tests(self):
        """Run all tests"""
        print("🧪 COMPREHENSIVE MEDIVOTE SERVICE MANAGER TEST SUITE")
        print("=" * 60)
        
        # Start service manager
        if not self.start_service_manager():
            print("❌ Failed to start service manager. Aborting tests.")
            return
...
if __name__ == "__main__":
    test_suite = ServiceManagerTestSuite()
